...
function preview(text?: string | null, max = 3000): string {
  if (!text) return "(empty)";
  return text.length > max ? `${text.slice(0, max)}\n... [truncated]` : text;
}

async function recordPlainFetch(url: string): Promise<string> {
...
async function recordPaidCapture(url: string): Promise<string> {
  ...
  if (!requirement) {
    throw new Error("Payment required response did not include any accepted payments");
  }
  ...
  const data = (await paidResponse.json()) as CaptureLogResponse;
...
writeFileSync(
  ...
  [
    `# Demo Output`,
    ``,
    `Generated by \`node dist/saveDemo.js ${targetUrl}\`.`,
    ``,
