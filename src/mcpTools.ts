...
import { z } from "zod";
import { createArcPaymentClient, getClientPrivateKey } from "./arc.js";
...
const SERVER_URL =
  process.env.EYEZ_URL || `http://localhost:${process.env.PORT || 3001}`;
...
async function paidCapture(url: string): Promise<PaidCaptureResult> {
  ...
  if (!client || !httpClient) {
    throw new Error(
      "EVM_PRIVATE_KEY or ARC_PRIVATE_KEY not set - cannot make x402 payments",
    );
  }
...
export function createEyezMcpServer(): McpServer {
  ...
  server.tool(
    ...
    "Capture a JavaScript-heavy webpage using a headless browser. Pays $0.001 USDC on Arc per request via x402. Use this when standard HTTP fetch returns empty content from SPAs, Twitter/X, LinkedIn, DeFi apps, or Cloudflare-protected sites.",
    {
      url: z
        .string()
        .url()
        .describe("The URL to capture (must be public http/https)"),
    },
    async ({ url }) => {
      try {
